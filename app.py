...
def get_metadata():
    ...
    for source in sources:
        ...
        for source, info in all_data.items():
            if source in ['gleif', 'latvia', 'register']:
                if info.get('json_zip'):
                    entry = [info.get('title'), info.get('update_date'), info['json_zip']]
                    lis.append(entry)
                else:
                    print(info.get('title')+ ' ' + info.get('update_date'))
